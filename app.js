...
function home() {
  return `<article class="page">
    ...
    <section class="intro"><div><span class="eyebrow">Sobre esta bitácora</span></div><div><p class="lead">Este sitio reúne mi proceso durante la materia: las lecturas que acompañan cada tema, los ejercicios realizados en clase y los trabajos que sintetizan lo aprendido durante el semestre.</p></div><aside>Un archivo vivo para observar cómo cambia mi manera de leer, hacer y aprender arquitectura.</aside><div></div></section>
    <section class="journey"><div class="journey-grid">
      ${[["01","Lecturas","Lo que leo","lecturas"],["02","Ejercicios","Lo que hago","ejercicios"],["03","Trabajos finales","Lo que aprendo","trabajos"]].map(([n,t,p,r]) => `<a class="journey-card" href="#/${r}"><span class="index-no">${n}</span><div class="shape" aria-hidden="true"></div><div><h3>${t}</h3><p>${p}</p></div></a>`).join("")}
    </div></section>
  </article>`;
}

function readings() {
  return `<article class="page">${pageHead("01 / 04", "Lecturas", "Un archivo de los textos trabajados durante el semestre: sus ideas principales, conceptos y las interpretaciones que construyo a partir de ellos.")}
...
function exercises() {
  return `<article class="page">${pageHead("02 / 04", "Ejercicios de clase", "Una galería del hacer: exploraciones geométricas, composiciones y procesos desarrollados dentro del aula.")}
...
function finals() {
  return `<article class="page">${pageHead("03 / 04", "Trabajos finales", "Proyectos que reúnen el proceso completo: desde el planteamiento inicial y las decisiones de diseño hasta el resultado y la reflexión final.")}
...
function detail(kind, id) {
  ...
  const backLabel = { lecturas: "Lecturas", ejercicios: "Ejercicios de clase", trabajos: "Trabajos finales" }[kind];
  ...
  if (kind === "lecturas" && item.information) {
    ...
    labels = kind === "trabajos" ? ["Descripción", "Concepto", "Proceso", "Resultado final", "Reflexión"] : ["Objetivo", "Proceso", "Resultado", "Explicación", "Reflexión personal"];
